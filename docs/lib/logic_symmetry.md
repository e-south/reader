...
A four-state summary transform and its companion plot place each response system
(per **design × batch**) at coordinates:
...
Reader materializes the calculation through a pipeline transform as the
validated `logic_symmetry/table` dataframe record. The selected protocol-owned
plot reads that immutable record and registers a separate figure bundle.

---

## Conceptual background

[Cox *et al.* (2007)](https://www.embopress.org/doi/full/10.1038/msb4100187) summarized dual-input response behavior with three numbers: regulatory range (`r`), logic type (`l`), and asymmetry (`a`). Their method sorts the four measured responses (`b1 ≤ b2 ≤ b3 ≤ b4`) and assumes monotone behavior. That represents AND/OR/SIG/SLOPE within a triangular region, but not XOR, NAND, NOR, or XNOR because sorting discards input identity.
...
8. Render and register the selected figure formats; optionally add ideals.
...
* A `logic_symmetry/table` dataframe record with one row per
  `(design_by…, batch)` and contract-validated metrics.
* A manifest-backed figure bundle selected by the protocol-owned
  `logic_symmetry` figure id and derived from that exact record revision.
...
* Select the Reader figure to compile the supporting summary transform and the
  manifest-backed `(L, A)` plot in one canonical plan. Add an explicit export
  only when another file representation is needed.
