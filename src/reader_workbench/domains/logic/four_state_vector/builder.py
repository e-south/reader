"""Pure four-state vector builder: tidy data to vector."""
...
from .config import FourStateVectorConfig, load_four_state_vector_config
...
def _assert_same_times(time_a: float | None, time_b: float | None) -> None:
    ...
    if time_a is None or time_b is None:
        raise ValueError("four-state vector: logic and intensity selections produced different times.")
    ta = float(time_a)
    tb = float(time_b)
    if not np.isclose(ta, tb, rtol=0, atol=1e-9):
        raise ValueError(f"four-state vector: logic and intensity channels selected different times: {ta} vs {tb}")


def _attach_sequence(vector: pd.DataFrame, tidy_df: pd.DataFrame, *, design_by: list[str]) -> pd.DataFrame:
    # If vector already has a usable 'sequence' column, keep it.
    if "sequence" in vector.columns and vector["sequence"].notna().any():
        return vector
    ...
    merged = vector.merge(seq_map, on=idx_cols, how="left", suffixes=("", "_ref"))
...
def _reorder_and_filter(
    ...
    *,
    ...
    if label_col not in df.columns:
        raise ValueError(f"four-state vector: expected label column '{label_col}' in vector output.")
...
class FourStateVectorBuildResult:
    vector: pd.DataFrame
    log: dict[str, Any]
...
def build_four_state_vector_from_tidy(
    tidy_df: pd.DataFrame, xform_cfg: Mapping[str, Any]
) -> FourStateVectorBuildResult:
    """
    Shared four-state vector builder for pipeline + notebooks.
    Returns vector + log payload + selection diagnostics without writing to disk.
    """
    cfg = load_four_state_vector_config(xform_cfg)
    ...
    sel_logic = cornerize_and_aggregate(
        ...
        preferred_treatment_column=cfg.treatment_column,
    ...
    # 4) Compute vector (logic from logic channel, y* from intensity channel + anchors)
    ref_raw = resolve_reference_design_id(tidy_df, design_by=cfg.design_by, ref_label=cfg.reference.design_id)
    vector = compute_four_state_vector(
    ...
    if sel_logic.chosen_time is not None:
        vector["time_selected_h"] = float(sel_logic.chosen_time)
        vector["time_selected_h"] = pd.to_numeric(vector["time_selected_h"], errors="coerce").astype(float)

    # 4b) Attach 'sequence' if available in the tidy inputs
    vector = _attach_sequence(vector, tidy_df, design_by=cfg.design_by)
    # 4b.5) Persist intensity log2 offset delta for downstream enforcement
    if "intensity_log2_offset_delta" not in vector.columns:
        vector["intensity_log2_offset_delta"] = float(cfg.log2_offset_delta)
    ...
    for col in cfg.carry_metadata or []:
        if col in tidy_df.columns and col not in vector.columns:
            meta = tidy_df[idx_cols + [col]].dropna(subset=[col]).drop_duplicates(subset=idx_cols, keep="first")
            vector = vector.merge(meta, on=idx_cols, how="left", validate="m:1")

    # 4c) Reference provenance columns
    if "reference_design_id" not in vector.columns:
        vector["reference_design_id"] = ref_raw

    # 4d) Apply requested filtering and column order
    vector_out = _reorder_and_filter(
        vector,
        design_by=cfg.design_by,
    ...
    r_desc = vector_out["r_logic"].describe().to_dict() if "r_logic" in vector_out.columns else {}
    flat_count = 0
    flat_fraction = 0.0
    flat_samples: list[str] = []
    if "flat_logic" in vector_out.columns and len(vector_out) > 0:
        flat_mask = vector_out["flat_logic"].astype(bool)
        flat_count = int(flat_mask.sum())
        flat_fraction = float(flat_count) / float(len(vector_out)) if len(vector_out) else 0.0
        label_col = cfg.design_by[0] if cfg.design_by else None
        if label_col and label_col in vector_out.columns and flat_count > 0:
            flat_samples = vector_out.loc[flat_mask, label_col].astype(str).dropna().drop_duplicates().head(5).tolist()

    log_payload: dict[str, Any] = {
        "operation": "four_state_vector",
        "design_by": cfg.design_by,
        "time_column": cfg.time_column,
        "channels": {
    ...
    return FourStateVectorBuildResult(
        vector=vector_out,
        log=log_payload,
        cfg=cfg,
        sel_logic=sel_logic,
