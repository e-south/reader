"""Selection and rendering for the canonical four-state event-window summary."""
...
class FourStateEventWindowSummaryMatrix:
...
def four_state_event_window_summary_matrix(
    ...
    *,
    ...
    if isinstance(maximum_rows, bool) or not isinstance(maximum_rows, int) or maximum_rows < 1:
        raise ValueError("four-state event-window summary maximum_rows must be a positive integer")
    ...
    if selected.empty:
        raise ValueError(
            f"four-state event-window plot has no non-reference rows for reduction {primary_reduction_id!r} "
            "under the configured experiment_ids/design_ids filters"
        )
    if len(selected) > maximum_rows:
        raise ValueError(
            f"four-state event-window summary selected {len(selected)} rows, exceeding maximum_rows={maximum_rows}; "
    ...
    if not np.isfinite(values).all():
        raise ValueError("four-state event-window summary requires finite component values")

    return FourStateEventWindowSummaryMatrix(
...
def render_four_state_event_window_summary(
    designs: pd.DataFrame,
    *,
    primary_reduction_id: str,
    title: str,
...
def _identity_filter(values: list[str] | tuple[str, ...] | None, *, label: str) -> tuple[str, ...] | None:
    ...
    if not normalized or any(not value for value in normalized):
        raise ValueError(f"four-state event-window summary {label} must contain non-empty identities")
    if len(normalized) != len(set(normalized)):
        raise ValueError(f"four-state event-window summary {label} must not contain duplicates")
    return normalized
