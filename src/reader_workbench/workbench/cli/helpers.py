...
def load_job_models(job_path: Path, *, runtime: ReaderRuntime | None = None) -> tuple[ReaderSpec, WorkbenchDecl]:
    runtime = runtime or _load("reader_workbench.runtime").builtin_runtime()
    spec = _load("reader_workbench.workbench.config").ReaderSpec.load(job_path)
    return spec, _load("reader_workbench.workbench.decl").build_workbench_decl(
...
def resolve_pipeline_step_id(decl: WorkbenchDecl, which: str, *, job_path: Path | None = None) -> str:
    ...
    pipeline = list(_load("reader_workbench.workbench.graph").resolve_workbench(decl).pipeline)
...
def spec_to_dict(spec_obj) -> dict:
    ...
    return {
        ...
        "reads": {
            key: _load("reader_workbench.workbench.graph").input_ref_to_dict(value)
            for key, value in (spec_obj.reads or {}).items()
        },
        "with": dict(spec_obj.with_ or {}),
        "writes": {
            key: _load("reader_workbench.workbench.graph").output_ref_to_dict(value)
