...
def ls(
    ...
    inspection_common = _load("reader_workbench.workbench.inspection.common")
    inspection_inventory = _load("reader_workbench.workbench.inspection.inventory")
    inspection_readiness = _load("reader_workbench.workbench.inspection.readiness")
    ...
    runtime = _load("reader_workbench.runtime").builtin_runtime() if details else None
    for idx, config_path in job_inventory:
        ...
        try:
            if details:
                ...
                spec = _load("reader_workbench.workbench.config").ReaderSpec.load(config_path)
...
def inspect(
    ...
    for renderable in _load("reader_workbench.workbench.inspection.reports").experiment_inspect_renderables(
        payload=payload,
...
def explain(
    ...
    try:
        ...
        if fmt == "json":
            emit_json(
                _load("reader_workbench.workbench.inspection.experiments").experiment_explain_payload(
                    job_path=job_path, spec=spec, decl=decl, runtime=runtime
                )
            )
            return
        _load("reader_workbench.workbench.engine").explain(decl, console=shared.console, runtime=runtime)
    except ReaderError as err:
        handle_reader_error(err)
...
def validate(
    ...
    try:
        ...
        fmt = normalize_output_format(format)
        if fmt == "json":
            summary = _load("reader_workbench.workbench.engine").validation_summary(
            ...
            emit_json(
                _load("reader_workbench.workbench.inspection.validation").validation_surface_payload(
            ...
            return
        summary = _load("reader_workbench.workbench.engine").validate(
            decl,
...
def config(
    ...
    if fmt == "json":
        runtime = _load("reader_workbench.runtime").builtin_runtime()
        emit_json(
            _load("reader_workbench.workbench.inspection.experiments").experiment_config_json_payload(
                job_path=job_path, spec=spec, decl=decl, runtime=runtime
            )
        )
        return
    if fmt == "yaml":
        ...
        materialized = _load("reader_workbench.workbench.graph").materialize_workbench(decl)
        payload["compiled"] = {
...
def run(
    ...
    try:
        _load("reader_workbench.workbench.engine").normalize_log_level(log_level)
    except ReaderError as err:
        handle_reader_error(err)
    if not dry_run:
    ...
    if only:
        ...
        try:
            ...
            if dry_run and fmt == "json":
                emit_json(
                    _load("reader_workbench.workbench.inspection.experiments").experiment_run_dry_run_payload(
                        job_path=job_path,
                        spec=spec,
                        decl=decl,
                        runtime=runtime,
                ...
                return
            _load("reader_workbench.workbench.engine").run_job(
                job_path,
                resume_from=only,
                until=only,
                dry_run=dry_run,
