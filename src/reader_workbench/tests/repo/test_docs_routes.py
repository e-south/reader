...
from reader_workbench.maintenance import docs as check_docs
...
def test_root_readme_is_a_human_first_landing_page() -> None:
    ...
    assert "uv tool install git+https://github.com/e-south/reader.git" in text
    assert "python -m pip install reader-workbench" not in text
    assert "uv tool install ." not in text
    assert "python -m pip install ." not in text
    assert "not yet been published to PyPI" in text
    assert "\nreader demo\n" in text
    assert "\nreader protocols\n" in text
...
def test_data_operations_plan_routes_to_machine_readable_registry() -> None:
    guide_path = REPO_ROOT / "docs" / "guides" / "data_operations_plan.md"
    text = guide_path.read_text(encoding="utf-8")
    assert "../../src/reader_workbench/workbench/dop/" in text
