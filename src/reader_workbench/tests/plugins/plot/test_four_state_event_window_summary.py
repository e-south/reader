...
from reader_workbench.plugins.plot.four_state_event_window_summary import (
    FourStateEventWindowSummaryCfg,
    FourStateEventWindowSummaryPlot,
)
...
def test_four_state_event_window_summary_plot_adapts_figure_metadata() -> None:
    cfg = FourStateEventWindowSummaryCfg(
        primary_reduction_id="event",
        experiment_ids=["source_b"],
        design_ids=["design_b"],
        maximum_rows=1,
        title="Selected four-state event-window",
        filename="summary",
        format=["png", "pdf"],
        dpi=144,
    )

    rendered = FourStateEventWindowSummaryPlot().render(None, {"designs": _designs_frame()}, cfg)
    ...
    assert rendered[0].fig.get_suptitle() == "Selected four-state event-window"
