...
from reader_workbench.plugins.plot.four_state_vector_diagnostic import (
    FourStateVectorDiagnosticCfg,
    FourStateVectorDiagnosticPlot,
)
...
def _inputs() -> dict[str, pd.DataFrame]:
    ...
    vector = pd.DataFrame.from_records(
        [
            {
                "design_id": design_id,
                "time_selected_h": 1.0,
    ...
    return {"df": pd.DataFrame.from_records(rows), "vector": vector}


def test_four_state_vector_diagnostic_declares_both_persisted_inputs() -> None:
    ports = FourStateVectorDiagnosticPlot.input_ports()

    assert ports["df"].contract == "plate_reader.annotated.v1"
    assert ports["vector"].contract == "logic.four_state_vector.v1"


@pytest.mark.parametrize("legacy_key", ["trajectory_ci", "trajectory_bootstraps"])
def test_four_state_vector_diagnostic_rejects_legacy_interval_keys(legacy_key: str) -> None:
    with pytest.raises(ValidationError, match=legacy_key):
        FourStateVectorDiagnosticCfg(state_map_ref="states", **{legacy_key: 10})


def test_four_state_vector_diagnostic_defaults_to_one_artifact_per_persisted_design() -> None:
    figures = FourStateVectorDiagnosticPlot().render(
        _context(),
        _inputs(),
        FourStateVectorDiagnosticCfg(
    ...
    assert [item.filename for item in figures] == [
        "four_state_vector_diagnostic--design_a",
        "four_state_vector_diagnostic--design-b",
    ]
    assert all(item.ext == "png" for item in figures)
    assert all(item.description and "persisted vector" in item.description for item in figures)
    for item in figures:
        plt.close(item.fig)


def test_four_state_vector_diagnostic_rejects_colliding_design_filename_slugs() -> None:
    inputs = _inputs()
    inputs["df"]["design_id"] = inputs["df"]["design_id"].replace({"design-b": "design_a"})
    inputs["vector"]["design_id"] = inputs["vector"]["design_id"].replace({"design-b": "design_a"})

    with pytest.raises(ExecutionError, match="duplicate artifact filenames"):
        FourStateVectorDiagnosticPlot().render(
            _context(),
            inputs,
            FourStateVectorDiagnosticCfg(
                state_map_ref="states",
...
def test_four_state_vector_diagnostic_runtime_persists_one_normal_plot_bundle(tmp_path: Path) -> None:
    ...
    declaration = WorkbenchDecl(
        experiment=ExperimentDecl(
            id="four_state_vector-diagnostic", title="four-state vector diagnostic", lifecycle="active", root=tmp_path
        ),
    ...
    record = store.latest_record("plot:four_state_vector_diagnostic")
    assert record is not None
    assert {item.ref.record_id for item in record.inputs} == {
        "promote_to_tidy_plus_map/df",
        "four_state_vector/vector",
    }
    assert (outputs / "plots" / "four_state_vector_diagnostic--design-b.png").is_file()
