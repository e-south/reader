...
from reader_workbench.workbench.registry import Plugin, PluginConfig, load_plugin_catalog
...
def test_load_plugin_catalog_registers_validator_plugins(monkeypatch) -> None:
    def _entry_points(*, group: str):
        if group == "reader_workbench.plugins":
            return [_EntryPoint()]
        return []
...
def test_load_plugin_catalog_does_not_import_unrequested_external_categories(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        md,
        "entry_points",
        lambda *, group: [_UnrequestedEntryPoint()] if group == "reader_workbench.plugins" else [],
    )
...
def test_validator_only_catalog_does_not_import_unrequested_builtin_categories(monkeypatch) -> None:
    unrelated_plugin_prefixes = (
        "reader_workbench.plugins.export.",
        "reader_workbench.plugins.ingest.",
        "reader_workbench.plugins.plot.",
        "reader_workbench.plugins.transform.",
    )
    manifest_prefix = "reader_workbench.plugins.manifests."
