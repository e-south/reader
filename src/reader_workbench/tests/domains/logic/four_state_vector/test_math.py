"""Tests for four-state vector compute_four_state_vector (up to the 8-vector)."""
...
from reader_workbench.domains.logic.four_state_vector.math import compute_four_state_vector
...
def test_logic_minmax_basic_and_r_logic():
    ...
    assert np.isclose(out["r_logic_min"], 1.0, atol=1e-12)
    assert np.isclose(out["r_logic_max"], 8.0, atol=1e-12)
    assert np.isclose(out["logic_span_log2"], 3.0, atol=1e-12)
    assert out["r_logic_corner_min"] == "00"
    assert out["r_logic_corner_max"] == "11"
...
def test_logic_scale_invariance():
    ...
    va = compute_four_state_vector(
        points_logic=pts_logic_a, points_intensity=pts_int, per_corner_intensity=per_corner, **kwargs
    )
    vb = compute_four_state_vector(
        points_logic=pts_logic_b, points_intensity=pts_int, per_corner_intensity=per_corner, **kwargs
    )
...
def test_intensity_alpha_delta_eps_abs_and_log_guard():
    ...
    outA = compute_four_state_vector(
        points_logic=pts_logic,
        points_intensity=pts_int,
        per_corner_intensity=per_corner,
        design_by=["design_id"],
    ...
    outB = compute_four_state_vector(
        points_logic=pts_logic,
        points_intensity=pts_int,
        per_corner_intensity=per_corner,
        design_by=["design_id"],
    ...
    outC = compute_four_state_vector(
        points_logic=pts_logic,
        points_intensity=pts_int,
        per_corner_intensity=per_corner,
        design_by=["design_id"],
    ...
    outD0 = compute_four_state_vector(
        points_logic=pts_logic,
    ...
    outD1 = compute_four_state_vector(
        points_logic=pts_logic,
        points_intensity=pts_int_nonzero,
        per_corner_intensity=per_corner,
        design_by=["design_id"],
...
def test_intensity_reference_stat_mean_vs_median():
    ...
    out_median = compute_four_state_vector(
        points_logic=pts_logic,
        points_intensity=pts_int,
        per_corner_intensity=per_corner,
        design_by=["design_id"],
    ...
    out_mean = compute_four_state_vector(
        points_logic=pts_logic,
        points_intensity=pts_int,
        per_corner_intensity=per_corner,
        design_by=["design_id"],
...
def test_compute_four_state_vector_applies_eps_and_alpha():
...
def test_vector_contract_critical_dtypes_are_numeric():
    pts_logic = pd.DataFrame([{"design_id": "X", "b00": 1, "b10": 2, "b01": 4, "b11": 8}])
    pts_int = pd.DataFrame([{"design_id": "X", "b00": 10, "b10": 10, "b01": 10, "b11": 10}])
    per_corner = pd.DataFrame([{"design_id": "REF", "corner": c, "y_mean": 5} for c in ("00", "10", "01", "11")])
