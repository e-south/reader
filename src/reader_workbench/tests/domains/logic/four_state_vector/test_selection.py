...
from reader_workbench.domains.logic.four_state_vector.reference import resolve_reference_design_id
from reader_workbench.domains.logic.four_state_vector.selection import cornerize_and_aggregate
...
def test_cornerize_aggregates_observations_for_logic_and_intensity():
    ...
    rows = []
    # Logic-channel values set so means are 1, 2, 3, 4 over two observations.
    ...
    for pos, corner in zip(["A1", "B1", "C1", "D1"], corners, strict=False):
        ...
        for observation_index in range(2):
            rows.append(
                {
                    "position": f"{pos}-{observation_index}",
                    "time": 12.0,
                    "channel": "YFP/CFP",
                    "value": vals_logic[corner][observation_index],
            ...
            rows.append(
                {
                    ...
                    "time": 12.0,
                    "channel": "YFP/OD600",
                    "value": vals_int[corner][observation_index],
                    "treatment": TMAP[corner],
                    "design_id": "G1",
                }
            )
...
def test_missing_reference_design_id_raises_in_compute_four_state_vector():
...
def _base_four_state_vector_cfg():
    return {
...
def test_load_four_state_vector_config_requires_design_id():
    cfg = _base_four_state_vector_cfg()
    cfg["design_by"] = ["genotype"]
    with pytest.raises(ValueError, match="design_by must start with 'design_id'"):
        load_four_state_vector_config(cfg)


def test_load_four_state_vector_config_rejects_unknown_reference_field():
    cfg = _base_four_state_vector_cfg()
    cfg["reference"] = {"unexpected": "value"}
    with pytest.raises(ValueError, match="four_state_vector.reference has unsupported setting: unexpected"):
        load_four_state_vector_config(cfg)
