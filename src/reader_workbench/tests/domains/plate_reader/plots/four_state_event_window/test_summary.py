...
from reader_workbench.domains.plate_reader.plots.four_state_event_window.summary import (
    COMPONENT_COLUMNS,
...
def test_four_state_event_window_component_order_is_canonical() -> None:
    assert COMPONENT_COLUMNS == ("r00", "r10", "r01", "r11", "b00", "b10", "b01", "b11")


def test_four_state_event_window_summary_matrix_selects_and_labels_rows() -> None:
    summary = four_state_event_window_summary_matrix(_designs_frame(), primary_reduction_id="event")
...
def test_four_state_event_window_summary_matrix_applies_explicit_identity_filters() -> None:
    summary = four_state_event_window_summary_matrix(
        _designs_frame(),
...
def test_four_state_event_window_summary_matrix_rejects_selection_over_row_budget() -> None:
    designs = pd.concat(
...
def test_four_state_event_window_summary_matrix_rejects_empty_selection() -> None:
    with pytest.raises(ValueError, match="no non-reference rows for reduction 'missing'"):
        four_state_event_window_summary_matrix(_designs_frame(), primary_reduction_id="missing")


def test_four_state_event_window_summary_matrix_rejects_nonfinite_values() -> None:
    designs = _designs_frame()
    designs.loc[0, "r00"] = np.inf

    with pytest.raises(ValueError, match="requires finite component values"):
        four_state_event_window_summary_matrix(designs, primary_reduction_id="event")


def test_four_state_event_window_summary_uses_separate_zero_centered_family_scales() -> None:
    figure = render_four_state_event_window_summary(
        _designs_frame(),
        primary_reduction_id="event",
        title="Four-state event-window summary",
    )
