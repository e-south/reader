...
from reader_workbench.domains.plate_reader.analysis.four_state_event_window.contracts import QualitySpec, ReductionSpec
from reader_workbench.domains.plate_reader.analysis.four_state_event_window.reduction import (
    four_state_event_window_temporal_spec,
)
from reader_workbench.domains.time_series import reduce_temporal_trace
...
def _spec(method: str, *, start_h: float = 0.0, end_h: float = 2.0):
    ...
    return four_state_event_window_temporal_spec(reduction, quality)
...
def test_four_state_event_window_adapter_matches_locked_numerical_reference(method: str) -> None:
    ...
    summary = reduce_temporal_trace(
        ...
        spec=spec,
        origin_h=7.0,
        trace_id=f"locked-parity:{method}",
    )
...
def test_four_state_event_window_adapter_preserves_censor_provenance_projection() -> None:
    summary = reduce_temporal_trace(
        np.asarray([0.0, 1.0, 2.0]),
        np.asarray([1.0, 2.0, 4.0]),
        spec=_spec("geometric_time_mean", start_h=0.5, end_h=1.5),
