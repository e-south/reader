...
ANALYSIS_PACKAGE = DOMAIN_ROOT / "analysis" / "four_state_event_window"
PLOT_PACKAGE = DOMAIN_ROOT / "plots" / "four_state_event_window"
...
def test_four_state_event_window_roles_have_distinct_packages() -> None:
...
def test_four_state_event_window_dependencies_point_outward_from_core() -> None:
...
def test_four_state_event_window_domain_has_no_four_state_vector_or_workbench_imports() -> None:
    source = "\n".join(
...
def test_four_state_event_window_never_calls_reference_relative_fluorescence_brightness() -> None:
    source = "\n".join(
        path.read_text(encoding="utf-8")
        for package in (ANALYSIS_PACKAGE, PLOT_PACKAGE)
        for path in package.rglob("*.py")
...
def test_four_state_event_window_public_analysis_uses_signal_value_vocabulary() -> None:
    source = "\n".join(
