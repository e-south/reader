...
class FourStateEventWindowSummaryCfg(PluginConfig):
    ...
    title: str = Field(default="Four-state event-window summary", min_length=1)
    filename: str = Field(default="four_state_event_window_summary", min_length=1)
    format: list[Literal["png", "pdf", "svg"]] = Field(default_factory=lambda: ["png"], min_length=1)
    dpi: int = Field(default=300, ge=1)


class FourStateEventWindowSummaryPlot(FigurePlotPlugin):
    ConfigModel = FourStateEventWindowSummaryCfg

    @classmethod
    def input_ports(cls):
        return {"designs": dataframe_input("designs", "plate_reader.four_state_event_window.designs.v4")}

    def render(self, ctx, inputs, cfg):
        from reader_workbench.domains.plate_reader.plots.four_state_event_window.summary import (  # noqa: PLC0415
