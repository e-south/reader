...
from reader_workbench.domains.plate_reader.analysis.response_window.materialize import materialize_experiment
from reader_workbench.domains.plate_reader.analysis.response_window.sources import build_experiment_source
...
class ResponseWindowCfg(PluginConfig):
    source: dict[str, Any]
    event: dict[str, Any]
...
class ResponseWindowTransform(Plugin):
    ...
    def run(self, ctx, inputs, cfg):
        spec = ResponseWindowAnalysisSpec.from_mapping(cfg.model_dump())
        ...
        if len(set(experiment_ids)) != len(experiment_ids):
            raise ValueError("response-window source experiments must be unique")
        for label, collection in (("magnitude", magnitude), ("trajectory", trajectory)):
            ...
            if observed != experiment_ids:
                raise ValueError(
                    f"response-window {label} source order must match response source experiments: "
