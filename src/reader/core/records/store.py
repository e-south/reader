...
from reader.core.workbench import (
...
def _empty_catalog() -> dict[str, Any]:
    return {"schema_version": 1, "latest": {}, "history": {}}
...
class RecordStore:
    ...
    def persist_dataframe(
        ...
        *,
        ...
        self.ensure_layout()
        base_name = f"{producer_id}.{producer_uses.replace('/', '_')}"
        ...
        if validate_contract and contract_id != "none":
            ...
            if contract is None:
                raise RecordError(f"Unknown contract id {contract_id!r}")
            validate_df(df, contract, where=f"{producer_id}:{out_name}")
        df.to_parquet(data_path, index=False)
        record = DataFrameArtifactRecord(
            record_id=record_id,
            kind="dataframe_artifact",
            producer=RecordProducer(kind="pipeline", id=producer_id, uses=producer_uses),
