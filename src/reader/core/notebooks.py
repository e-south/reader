...
EXPERIMENT_SFXI_EDA_TEMPLATE = '''import marimo
...
def _(Path, ReaderSpec):
    ...
    def _resolve_outputs(spec) -> tuple[Path, Path]:
        ...
        return outputs_path, exports_path

    exp_dir = _find_experiment_root(Path(__file__).resolve())
    spec = _load_spec(exp_dir)
    outputs_dir, exports_dir = _resolve_outputs(spec)
    exp_meta = {
...
The vec8 here uses the same `transform/sfxi` code and writes XLSX + JSON logs to the experiment's exports folder."""
    )
...
@app.cell(hide_code=True)
def _(mo, pd, sfxi_cfg, tidy_pd, np):
...
def _(
    ...
    try:
        sel_logic = cornerize_and_aggregate(
            ...
            target_time_h=float(time_slider.value),
            time_mode=str(time_mode.value),
    ...
    warn_note = sel_logic.time_warning
    time_note = (
        f"Selected snapshot time: {float(chosen_time):.3f} h"
        if chosen_time is not None
        else "Selected snapshot time: unavailable"
    )
    return subset_pd, sel_logic, sel_int, chosen_time, time_note, warn_note

@app.cell(hide_code=True)
def _(mo, time_note, warn_note):
    lines = [f"**{time_note}**"]
    if warn_note:
        lines.append(f"**Time tolerance warning:** {warn_note}")
    mo.md("## Snapshot selection\\n" + "\\n".join(lines))

@app.cell(hide_code=True)
def _(mo, sfxi_cfg, subset_pd):
    _cols = list(subset_pd.columns)
    if "treatment_alias" in _cols:
        treatment_col = "treatment_alias"
    elif "treatment" in _cols:
        treatment_col = "treatment"
    else:
        mo.stop(True, mo.md("SFXI plotting requires `treatment` or `treatment_alias` columns."))
    treatment_order = [sfxi_cfg.treatment_map[_k] for _k in ("00", "10", "01", "11")]
    return treatment_col, treatment_order
...
@app.cell(hide_code=True)
def _(np, pd, chosen_time, subset_pd, time_col, treatment_col, sfxi_cfg):
    ...
    if not _dfc.empty:
        ...
        _dfc["value"] = pd.to_numeric(_dfc["value"], errors="coerce")
        _dfc = _dfc.dropna(subset=[time_col, "value", treatment_col])
        _times = sorted({float(_v) for _v in _dfc[time_col].dropna().unique().tolist()})
        if _times:
            if chosen_time is None:
                time_snap = float(_times[-1])
            else:
                if np is not None:
                    _diffs = np.abs(np.array(_times, dtype=float) - float(chosen_time))
                    time_snap = float(_times[int(_diffs.argmin())])
                else:
                    _diffs = [abs(_t - float(chosen_time)) for _t in _times]
                    time_snap = float(_times[_diffs.index(min(_diffs))])
...
def _(
    ...
    if time_snap is not None:
        _snap_time = float(time_snap)
    elif chosen_time is not None:
        _snap_time = float(chosen_time)
    else:
        _snap_time = float(time_slider.value)
    ...
    ts_chart = alt.layer(_ts_band, _ts_line, _ts_rule, _ts_text).properties(
        width=_ts_width,
        height=_ts_height,
    )
...
@app.cell(hide_code=True)
def _(build_vec8_from_tidy, mo, sfxi_step_cfg, time_mode, time_slider, tidy_pd):
    cfg_payload = dict(sfxi_step_cfg)
    cfg_payload["target_time_h"] = float(time_slider.value)
    cfg_payload["time_mode"] = str(time_mode.value)
    try:
        vec8_result = build_vec8_from_tidy(tidy_pd, cfg_payload)
    except Exception as exc:
        mo.stop(True, mo.md(f"Vec8 computation failed: `{exc}`"))
    return vec8_result
...
def _(Path, exports_dir, mo, sfxi_cfg):
    ...
    export_button = mo.ui.run_button(label="Export vec8 (XLSX)", kind="success")
    ...
    mo.vstack(
        [
            ...
            mo.md("XLSX export uses `openpyxl` (included in reader core deps; run `uv sync` if missing)."),
        ]
    )
...
def _(export_button, export_path, json, log_name, mo, vec8_result):
    ...
    mo.md(f"Exported vec8 to `{export_path}` and log to `{log_path}`.")
