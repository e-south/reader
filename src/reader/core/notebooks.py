...
EXPERIMENT_EDA_TEMPLATE = """import marimo
...
def write_experiment_eda_notebook(target: Path, *, overwrite: bool = False) -> Path:
    if target.exists() and not overwrite:
        raise ConfigError(f"Notebook already exists: {target}")
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(EXPERIMENT_EDA_TEMPLATE, encoding="utf-8")
    return target
