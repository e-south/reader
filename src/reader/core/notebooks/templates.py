...
--------------------------------------------------------------------------------
<reader project>
src/reader/core/notebooks/templates.py
...
def _():
    ...
    from reader.core.records import discover_dataframe_records
...
@app.cell(hide_code=True)
def _(Path, ReaderSpec, resolve_workbench):
...
@app.cell(hide_code=True)
def _(mo, resolve_workbench, spec):
    sfxi_step = None
    for step in resolve_workbench(spec).pipeline:
...
def _(mo, spec, sfxi_step):
    ...
    if isinstance(logic_map_ref, str):
        logic_maps = getattr(spec.assay, "logic_maps", {}) or {}
        logic_spec = logic_maps.get(logic_map_ref)
        if logic_spec is None:
            mo.stop(True, mo.md(f"SFXI config error in `{sfxi_step_id}`: unknown logic_map_ref `{logic_map_ref}`."))
        if hasattr(logic_spec, "model_dump"):
            logic_spec = logic_spec.model_dump()
        sfxi_step_cfg = dict(sfxi_step_cfg)
        sfxi_step_cfg["treatment_map"] = dict(logic_spec.get("corners") or {})
        sfxi_step_cfg["treatment_case_sensitive"] = bool(logic_spec.get("case_sensitive", True))
    ...
    if missing:
        mo.stop(
            ...
            mo.md(
                ...
                "Choose the annotated plate-reader artifact (validator/to_tidy_plus_map) and see `docs/sfxi_vec8_in_reader.md`."
            ),
        )
    return required
