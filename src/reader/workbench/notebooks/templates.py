...
--------------------------------------------------------------------------------
<reader project>
src/reader/workbench/notebooks/templates.py
...
def _(mo, pipeline_step_ids, record_info, record_labels, record_note, record_warning):
    ...
    if not record_labels:
        note = record_note or "No datasets found. Run `reader run` first."
        mo.md(note)
...
def _(decl, mo, resolve_workbench):
    ...
    sfxi_step = None
    registry = builtin_runtime().plugins
...
def _(decl, mo, sfxi_step):
    sfxi_step_cfg = dict(getattr(sfxi_step, "with_", {}) or {})
    ...
    if isinstance(logic_map_ref, str):
        try:
            logic_spec = decl.experiment_semantics.assay.resolve_logic_map(ref=logic_map_ref)
        except Exception as exc:
