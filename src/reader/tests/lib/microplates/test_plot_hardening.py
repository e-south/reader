...
--------------------------------------------------------------------------------
<reader project>
src/reader/tests/test_plot_hardening.py

Direct library hardening tests for plot semantics.
--------------------------------------------------------------------------------
"""
...
from reader.tests.support import write_config
    ...
    ctx = SimpleNamespace(
        ...
        assay={
            "orders": {
                ...
                "screen_rows": {"column": "design_id_alias", "values": ["spyP/tetO", "spyP/CpxR"]},
            }
        },
    )
    df = pd.DataFrame(
...
def test_snapshot_heatmap_render_rejects_unknown_order_ref() -> None:
    ctx = SimpleNamespace(logger=logging.getLogger("reader.tests"), assay={"orders": {}})
    df = pd.DataFrame(
        {
            "time": [0.0],
            "channel": ["YFP/CFP"],
