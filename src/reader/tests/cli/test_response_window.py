...
def test_response_window_cli_exposes_explicit_lifecycle() -> None:
    ...
    for command in ("preflight", "build", "verify", "review"):
...
def test_response_window_preflight_json_names_failed_checks(monkeypatch, tmp_path: Path) -> None:
    result = _preflight(tmp_path, ready=False, missing_display_examples=("missing-design",))
    monkeypatch.setattr(response_window_cli, "preflight_response_window_request", lambda **_kwargs: result)
    ...
    assert "missing-design" in error["reason"]
