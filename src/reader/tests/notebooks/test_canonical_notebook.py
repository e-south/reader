...
from reader.workbench.notebooks import CANONICAL_NOTEBOOK_ID
...
def test_canonical_notebook_names_the_notebooks_optional_extra(tmp_path: Path) -> None:
    _, body = _render_canonical_notebook(tmp_path)
    assert "notebooks group" not in body
    if "Polars is required for notebook tables." in body:
        assert "Install the `notebooks` extra." in body
...
def test_generated_generic_eda_executes_without_assay_identity_columns(tmp_path: Path) -> None:
    if importlib.util.find_spec("marimo") is None:
        pytest.skip("Marimo is unavailable; install the `notebooks` extra to run notebook execution tests.")
