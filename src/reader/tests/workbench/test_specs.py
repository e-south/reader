...
def test_resolve_workbench_separates_plugin_steps_from_notebook_templates(tmp_path: Path) -> None:
    ...
    assert workbench.plots[0].id == "time_series"
    assert workbench.plots[0].plugin_category == "plot"
    assert workbench.exports[0].id == "crosstalk_pairs_csv"
...
def test_materialize_workbench_emits_pipeline_plot_export_and_notebook_sections(tmp_path: Path) -> None:
    ...
    assert [item["id"] for item in materialized["plots"]] == ["time_series"]
    assert [item["id"] for item in materialized["exports"]] == ["crosstalk_pairs_csv"]
...
def test_resolve_workbench_preserves_typed_recipe_provenance(tmp_path: Path) -> None:
    cfg = base_reader_config(
        ...
        protocol_deliverables={"plots": {"profile": "none"}},
        resources={"sample_map": {"kind": "file", "path": "./inputs/metadata.xlsx"}},
    )
...
def test_resolve_workbench_normalizes_compiled_bindings_to_typed_refs(tmp_path: Path) -> None:
    ...
    assert time_series.reads["df"] == RecordRef(record_id="ratio_yfp_od600/df")
    assert time_series.reads["blanks"] == RecordRef(record_id="blank/blanks")
...
def test_materialize_workbench_serializes_typed_refs_back_to_binding_dicts(tmp_path: Path) -> None:
    ...
    assert plots["time_series"]["reads"]["df"] == {"record": "ratio_yfp_od600/df"}
