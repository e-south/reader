...
from reader.domains.plate_reader.evidence.response_window import bundle as bundle_module
...
def test_verify_bundle_rejects_study_identity_drift(tmp_path: Path) -> None:
    ...
    manifest_path = root / "manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest["study_id"] = "another_collection"
    manifest_path.write_text(json.dumps(manifest), encoding="utf-8")
...
def _bundle_fixture(tmp_path: Path, *, study_id: str = "stress_ethanol_cipro_growth") -> Path:
    ...
    (root / "sources" / "experiment" / "config.yaml").write_text(
        "schema: reader/v8\nexperiment:\n  id: experiment\n  title: Example response experiment\n",
        encoding="utf-8",
    )
    source_digests = {
