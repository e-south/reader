...
def test_retron_presentation_exposes_decision_card_metric_specs_in_review_order() -> None:
    ...
    assert [spec.metric for spec in specs] == ["P_pre", "O_AUC"]
...
def test_retron_presentation_marks_expected_direction_positive_area_summary_text_explicitly() -> None:
    assert (
        retron_presentation.summary_metric_text_spec("O_AUC").lead
        == "Positive-area integral of the expected-direction post-stress ΔR after preload removal"
    )
    assert (
        retron_presentation.summary_metric_text_spec("O_abs_AUC").lead
        == "Positive-area integral of the expected-direction matched-tetO ΔR over the post-stress window"
    )


def test_retron_presentation_registers_cross_run_summary_labels() -> None:
    assert retron_presentation.summary_metric_label("O_AUC") == "Expected-direction post-stress area"
    assert retron_presentation.summary_metric_label("O_abs_AUC") == "Expected-direction total area"
