...
def test_logic_minmax_basic_and_r_logic():
    ...
    out = compute_vec8(
        ...
        eps_ratio=1e-12, eps_range=1e-15, eps_ref=1e-12, eps_abs=0.0,
...
def test_logic_flat_detection_exact():
    ...
    out = compute_vec8(
        ...
        design_by=["genotype"], batch_col="batch",
        reference_genotype="REF", reference_scope="batch", reference_stat="mean",
        eps_ratio=1e-9, eps_range=1e-12, eps_ref=1e-9, eps_abs=0.0,
        ref_add_alpha=0.0, log2_offset_delta=0.0,
    ).iloc[0]
...
def test_logic_flat_detection_within_eps_range():
    ...
    out = compute_vec8(
        ...
        design_by=["genotype"], batch_col="batch",
        reference_genotype="REF", reference_scope="batch", reference_stat="mean",
        eps_ratio=1e-12, eps_range=1e-10, eps_ref=1e-12, eps_abs=0.0,
        ref_add_alpha=0.0, log2_offset_delta=0.0,
    ).iloc[0]
    assert bool(out["flat_logic"]) is True
    assert np.allclose([out["v00"], out["v10"], out["v01"], out["v11"]], 0.25)
...
def test_logic_zeros_guard_and_expected_mapping():
    ...
    out = compute_vec8(
        ...
        design_by=["genotype"], batch_col="batch",
        reference_genotype="REF", reference_scope="batch", reference_stat="mean",
        eps_ratio=1e-3, eps_range=1e-12, eps_ref=1e-9, eps_abs=0.0,
        ref_add_alpha=0.0, log2_offset_delta=0.0,
    ).iloc[0]
...
def test_intensity_alpha_delta_eps_abs_and_log_guard():
    ...
    outB = compute_vec8(
        ...
        design_by=["genotype"], batch_col="batch",
        reference_genotype="REF", reference_scope="batch", reference_stat="mean",
        eps_ratio=1e-12, eps_range=1e-12, eps_ref=1e-12, eps_abs=1e-6,
        ref_add_alpha=0.0, log2_offset_delta=0.0,
    ).iloc[0]
    ...
    outC = compute_vec8(
        ...
        ref_add_alpha=0.0, log2_offset_delta=0.5,
    ).iloc[0]
    ...
    outD0 = compute_vec8(
        points_logic=pts_logic, points_intensity=pts_int_nonzero, per_corner_intensity=per_corner,
        design_by=["genotype"], batch_col="batch",
        reference_genotype="REF", reference_scope="batch", reference_stat="mean",
        eps_ratio=1e-12, eps_range=1e-12, eps_ref=1e-12, eps_abs=0.0,
        ref_add_alpha=0.0, log2_offset_delta=0.0,
    ...
    outD1 = compute_vec8(
        ...
        reference_genotype="REF", reference_scope="batch", reference_stat="mean",
        eps_ratio=1e-12, eps_range=1e-12, eps_ref=1e-12, eps_abs=0.0,
        ref_add_alpha=9.0, log2_offset_delta=0.0,
...
def test_intensity_batch_vs_global_reference_scope():
    ...
    out_global = compute_vec8(
        points_logic=pts_logic, points_intensity=pts_int, per_corner_intensity=per_corner,
        design_by=["genotype"], batch_col="batch",
        reference_genotype="REF", reference_scope="global", reference_stat="mean",
        eps_ratio=1e-12, eps_range=1e-12, eps_ref=1e-12, eps_abs=0.0,
        ref_add_alpha=0.0, log2_offset_delta=0.0,
    )
...
def test_intensity_reference_stat_mean_vs_median():
    ...
    out_median = compute_vec8(
        points_logic=pts_logic, points_intensity=pts_int, per_corner_intensity=per_corner,
        design_by=["genotype"], batch_col="batch",
        reference_genotype="REF", reference_scope="batch", reference_stat="median",
        eps_ratio=1e-12, eps_range=1e-12, eps_ref=1e-12, eps_abs=0.0,
        ref_add_alpha=0.0, log2_offset_delta=0.0,
    ).iloc[0]
...
def test_missing_anchor_raises():
    ...
    try:
        ...
        assert False, "Expected ValueError for missing anchor"
...
def test_multiple_genotypes_and_batches():
    ...
    assert np.allclose(
        y_row[["y00_star","y10_star","y01_star","y11_star"]].to_numpy(),
        0.0
    )
...
def test_flat_logic_is_boolean_dtype():
    ...
    per_corner= pd.DataFrame([{"genotype":"REF","batch":0,"corner":c,"y_mean":5} for c in ("00","10","01","11")])
