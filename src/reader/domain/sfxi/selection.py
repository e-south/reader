...
def _ensure_batch_col(df: pd.DataFrame, batch_col: str | None) -> tuple[pd.DataFrame, str]:
    if batch_col:
        if batch_col not in df.columns:
            raise ValueError(f"SFXI: missing batch column '{batch_col}'")
        try:
            pd.to_numeric(df[batch_col])
        except Exception as err:
            raise ValueError(f"SFXI: batch column '{batch_col}' must be numeric (0,1,2,...)") from err
        return df, batch_col
    if "batch" in df.columns:
        try:
            pd.to_numeric(df["batch"])
        except Exception as err:
            raise ValueError("SFXI: batch column 'batch' must be numeric (0,1,2,...)") from err
        return df, "batch"
    df = df.copy()
    df["batch"] = 0
    return df, "batch"
...
def cornerize_and_aggregate(
    ...
    *,
    ...
    df, batch_col = _ensure_batch_col(df, batch_col)
    ...
    group_cols = design_by + [batch_col] + ["corner"]
