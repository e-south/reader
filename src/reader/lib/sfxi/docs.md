...
`r_logic` reports the **dynamic range** of the **logic channel** values *before* log/min–max (how far apart the four corners are in linear space). We compute it **per (design × batch)** after an ε guard:
...
transformations:
  - type: sfxi
    ...
    time_tolerance_h: 0.25             # SOFT: exceeding this emits a warning and proceeds
    time_per_batch: true
