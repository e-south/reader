...
def prepare_single_reporter_diagnostics(
    ...
    *,
    ...
    for column in (resolved_condition, unit_column, observation_column, resolved_group):
...
def _prepare_partition(
    ...
    *,
    ...
    group_prefix = work[group_column].astype(str) if group_column is not None else None
    unit = work[unit_column].astype(str)
    work["__unit"] = unit if group_prefix is None else group_prefix.str.cat(unit, sep="::")
    observation = work[observation_column].astype(str)
    work["__observation"] = work["__unit"].str.cat(observation, sep="::")
    work["__segment"] = _segment_identity(work)

    trace_keys = ["__condition", "__unit", "__observation", "channel"]
    ...
    _require_aligned_channel_times(
        work,
        trace_identity=["__condition", "__unit", "__observation"],
        time_column=time_column,
        channels=(normalizer_channel, reporter_channel, ratio_channel),
    ...
    _require_complete_channels(
        kinetics,
        key_columns=["__segment", time_column, "__condition", "__unit"],
        channels=(normalizer_channel, reporter_channel, ratio_channel),
    ...
    _require_complete_channels(
        reduced,
        key_columns=["__condition", "__unit"],
