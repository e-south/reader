"""Strict configuration model for the SFXI transform."""
...
def _sub(obj: Mapping[str, Any], key: str, *, allowed: frozenset[str]) -> dict[str, Any]:
    ...
    if isinstance(value, Mapping):
        ...
        raise ValueError(f"sfxi.{key} must be a mapping.")
    unsupported = sorted(str(item) for item in payload if item not in allowed)
    if unsupported:
        noun = "setting" if len(unsupported) == 1 else "settings"
        raise ValueError(f"sfxi.{key} has unsupported {noun}: {', '.join(unsupported)}.")
    return payload


def _nonempty_text(value: object, *, field_name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise ValueError(f"sfxi.{field_name} must be a non-empty string.")
    return value.strip()


def _string_list(value: object, *, field_name: str) -> list[str]:
    if not isinstance(value, list) or any(not isinstance(item, str) or not item.strip() for item in value):
        raise ValueError(f"sfxi.{field_name} must be a list of non-empty strings.")
...
def load_sfxi_config(xform_cfg: Mapping[str, Any]) -> SFXIConfig:
    """
    Validate an explicit transform mapping and build an SFXIConfig.
    Requires explicit 'response.logic_channel' and 'response.intensity_channel'.
    """
    if not isinstance(xform_cfg, Mapping):
        raise ValueError("SFXI settings must be a mapping.")
    unsupported = sorted(str(key) for key in xform_cfg if key not in _CONFIG_KEYS)
    if unsupported:
        raise ValueError(f"Unsupported SFXI settings: {', '.join(unsupported)}.")
    ...
    if not isinstance(raw_treatment_map, Mapping):
        raise ValueError("sfxi.treatment_map must be a mapping.")
    ...
    if any(not isinstance(value, str) or not value.strip() for value in tmap.values()):
        raise ValueError("sfxi.treatment_map values must be non-empty strings.")
