...
--------------------------------------------------------------------------------
<reader project>
src/reader/plotters/logic_symmetry/main.py
...
def plot_logic_symmetry(
    ...
    **_ignored,
    ...
    if prep and bool(prep.get("enable", False)):
        ...
        from reader.processors.logic_symmetry_prep import prepare_for_logic_symmetry
    ...
    write_csv(csv_out, out_dir, base)
    save_plot(fig, out_dir, base, out_formats, dpi)

    LOG.info("✓ logic_symmetry wrote: %s.[%s] and %s.csv",
             str(out_dir / base), ",".join(out_formats), base)
